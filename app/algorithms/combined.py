    ...
    def generate_signal(
        ...
        if "ma_crossover" in modules:
            ...
            if len(df) >= slow + 2:
                ...
                if not ma_fast.dropna().empty and not ma_slow.dropna().empty:
                    ...
                    trend = "выше" if fc > sc else "ниже"
                    log_lines.append(("INFO",
                        f"MA{fast}={fc:.6f} {trend} MA{slow}={sc:.6f}"
                        + (" — 🟢 золотой крест!" if ma_buy else
                           " — 🔴 мёртвый крест!" if ma_sell else
                           f" — {'тренд вверх' if fc > sc else 'тренд вниз'}")
                    ))
            else:
                buy_signals.append(False)
                sell_signals.append(False)
                log_lines.append(("WARN", f"MA: мало свечей ({len(df)} < {slow + 2})"))
        ...
        if "rsi" in modules:
            ...
            if len(df) >= period + 2:
                ...
                if len(rsi_s) >= 2:
                    ...
                    if rc <= oversold:
                        zone = f"перепродан (≤{oversold:.0f})"
                    elif rc >= overbought:
                        zone = f"перекуплен (≥{overbought:.0f})"
                    else:
                        zone = "нейтральная зона"
                    log_lines.append(("INFO",
                        f"RSI({period})={rc:.1f} — {zone}"
                        + (" — 🟢 выход из перепроданности!" if rsi_buy else
                           " — 🔴 вошёл в перекупленность!" if rsi_sell else "")
                    ))
                else:
                    buy_signals.append(False)
                    sell_signals.append(False)
                    log_lines.append(("WARN", f"RSI: мало данных после dropna"))
            else:
                buy_signals.append(False)
                sell_signals.append(False)
                log_lines.append(("WARN", f"RSI: мало свечей ({len(df)} < {period + 2})"))

        if not buy_signals:
            state["_log"] = [("WARN", "Нет активных модулей — проверьте настройки бота")]
            return "HOLD", state
        ...
        if has_position:
            ...
            if sl_pct and entry_price:
                if current_price <= entry_price * (1 - float(sl_pct) / 100):
                    ...
                    log_lines.append(("SELL", f"🛑 Стоп-лосс: цена {current_price:.6f} упала ниже SL {sl_price:.6f} (−{sl_pct}%) — продаём"))
            ...
            if tp_pct and entry_price:
                if current_price >= entry_price * (1 + float(tp_pct) / 100):
                    ...
                    log_lines.append(("SELL", f"💰 Тейк-профит: цена {current_price:.6f} достигла TP {tp_price:.6f} (+{tp_pct}%) — продаём"))
                    state["_log"] = log_lines
                    return "SELL", state
            ...
            if trail_pct and max_price:
                if current_price <= max_price * (1 - float(trail_pct) / 100):
                    ...
                    log_lines.append(("SELL", f"📉 Трейлинг-стоп: откат от максимума {max_price:.6f} до {current_price:.6f} — продаём"))
                    state["_log"] = log_lines
                    return "SELL", state
            ...
            if do_sell:
                ...
                log_lines.append(("SELL", f"🔴 Сигнал на выход ({logic_str.strip(' +')}) — продаём по {current_price:.6f}"))
                state["_log"] = log_lines
                return "SELL", state

            # Still holding
            pnl_pct = (current_price - entry_price) / entry_price * 100 if entry_price else 0.0
            log_lines.append(("INFO", f"Позиция открыта: вход {entry_price:.6f}, сейчас {current_price:.6f}, P&L {pnl_pct:+.2f}%"))
            state["_log"] = log_lines
        ...
        # ── Entry ─────────────────────────────────────────────────────────
            ...
            if do_buy:
                ...
                logic_str = ("все модули согласны" if entry_logic == "AND"
                             else "один из модулей сигнализирует")
                log_lines.append(("BUY", f"🟢 Покупка ({logic_str}, логика {entry_logic}) — вход по {current_price:.6f}"))
                state["_log"] = log_lines
                return "BUY", state
